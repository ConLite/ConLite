...
// $Id: calendar.js 2 2011-07-20 12:00:48Z oldperl $
...
Calendar.resetBackground = function (el) {
    var columns = el.getElementsByTagName('td');
    for (var i = 1; i < columns.length; i++) {
        if (columns[i].className.match(/nweekend/g)) {
            Calendar.removeClass(columns[i], "nweekend");
            Calendar.addClass(columns[i], "weekend");
        } else if (columns[i].className.match(/weekend/g)) {
            Calendar.removeClass(columns[i], "weekend");
            Calendar.addClass(columns[i], "nweekend");
        }
        
        if (columns[i].className.match(/todayhilite/g)) {
            Calendar.removeClass(columns[i], "todayhilite");
            Calendar.addClass(columns[i], "today");
        } else if (columns[i].className.match(/today/g)) {
            Calendar.removeClass(columns[i], "today");
            Calendar.addClass(columns[i], "todayhilite");
        }
    }
}
...
Calendar.dayMouseDown = function(ev) {
	...
	cal.activeDiv = el;
	Calendar._C = cal;
	if (el.navtype != 300 && el.navtype != 400 && el.navtype != 200) with (Calendar) {
		if (el.navtype == 50) {
...
Calendar.prototype.create = function (_par) {
	...
	hh('helpButton', 1, 400).ttip = Calendar._TT["INFO"];
	this.title = hh("", title_length, 300);
	this.title.className = "title";
	if (this.isPopup) {
		this.title.ttip = Calendar._TT["DRAG_TO_MOVE"];
		this.title.style.cursor = "move";
		var returnCell = hh('cancelButton', 1, 200).ttip = Calendar._TT["CLOSE"];
	}
