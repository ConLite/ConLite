<?php
/**
 * Adapted version of editarea compressor for ConLite
 * 
 * @package EditArea
...
include_once dirname(dirname(dirname(__FILE__)))."/includes/startup.php";
// CONFIG
...
$compressor = new CL_Compressor($param);
...
class CL_Compressor {
    ...
    function __construct($param) {
        ...
        $this->cachepath = dirname(dirname(dirname(dirname(__FILE__)))) . "/data/cache/";
        if (isset($_GET['plugins'])) {
            $this->load_all_plugins = true;
            $this->full_cache_file = $this->cachepath . "edit_area_full_with_plugins.js";
            $this->gzip_cache_file = $this->cachepath . "edit_area_full_with_plugins.gz";
        } else {
            $this->load_all_plugins = false;
            $this->full_cache_file = $this->cachepath . "edit_area_full.js";
            $this->gzip_cache_file = $this->cachepath . "edit_area_full.gz";
        }
        ...
        if (file_exists($this->cache_file)) {
            ...
            if ($dir = opendir($this->cachepath)) {
                while (($file = readdir($dir)) !== false) {
                    if (is_file($this->cachepath . $file) && $file != "." && $file != "..")
                        $cache_date = max($cache_date, filemtime($this->cachepath . $file));
                }
                closedir($dir);
            }
            ...
            while (($file = readdir($dir)) !== false) {
                if ($file !== "." && $file !== ".." && ( $pos = strpos($file, '.js') ) !== false) {
                    ...
                    if (preg_match('@(\'|")DISPLAY_NAME\1\s*:\s*(\'|")(.*)\2@', $jsContent, $match)) {
                        $a_displayName[] = "'" . substr($file, 0, $pos) . "':'" . clHtmlSpecialChars($match[3], ENT_QUOTES) . "'";
                    }
                }
            }
            closedir($dir);
