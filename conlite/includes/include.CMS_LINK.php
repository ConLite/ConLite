 ...
 *
 *   $Id: include.CMS_LINK.php 312 2014-06-18 11:01:08Z oldperl $:
 * }}
 * 
 */
...
<table>
  <tr>
    <td rowspan="4"><img src="<?php print $cfg["path"]["contenido_fullhtml"].$cfg["path"]["images"] ?>spacer.gif" width="10" height="10"></td>
    <td><img src="<?php print $cfg["path"]["contenido_fullhtml"].$cfg["path"]["images"] ?>spacer.gif" width="10" height="10"></td>
    <td rowspan="4"><img src="<?php print $cfg["path"]["contenido_fullhtml"].$cfg["path"]["images"] ?>spacer.gif" width="10" height="10"></td>
  </tr>
  <tr>
    <td>
    <?php
        ...
        $aFileTypes = array('pdf','doc','ppt','xls','rtf','dot','docx','xlsx','pptx');
        $sAdditionalFileTypes = getEffectiveSetting("cms_typ", "additional_doc_types", false);
        
        if($sAdditionalFileTypes !== false && !empty($sAdditionalFileTypes)) {
            $aTmpArr = explode(",", $sAdditionalFileTypes);
            $aFileTypes = array_merge($aFileTypes, $aTmpArr);
        }
        $oUploadedFiles = new UploadCollection();
        $oUploadedFiles->setWhere('idclient', $client);
        $oUploadedFiles->setWhere('filetype', $aFileTypes, "in");
        $oUploadedFiles->setOrder('dirname, filename');
        $oUploadedFiles->query();
        
        /* @var Upload $oFile */
        while($oFile = $oUploadedFiles->next()) {
           $pdflink.= "<option value=\""
            .$oFile->get("dirname").$oFile->get("filename")."\">"
                   .$oFile->get("dirname").$oFile->get("filename")." ["
                   .$oFile->get("description")."]</option>"; 
        }
        unset($oFile);
        $pdflink.= "</SELECT>";
        ...
        $aFileTypes = array('png','gif','tif','jpg','jpeg','psd','pdd','iff','bmp','rle','eps','fpx','pcx','jpe','pct','pic','pxr','tga');
        $sAdditionalFileTypes = getEffectiveSetting("cms_typ", "additional_image_types", false);
        
        if($sAdditionalFileTypes !== false && !empty($sAdditionalFileTypes)) {
            $aTmpArr = explode(",", $sAdditionalFileTypes);
            $aFileTypes = array_merge($aFileTypes, $aTmpArr);
        }
        $oUploadedFiles->resetQuery();
        $oUploadedFiles->setWhere('idclient', $client);
        $oUploadedFiles->setWhere('filetype', $aFileTypes, "in");
        $oUploadedFiles->setOrder('dirname, filename');
        $oUploadedFiles->query();
       ...
       cInclude("includes","functions.upl.php");
        ...
        $form->add(i18n("Description"),"<TEXTAREA name=CMS_LINKDESCR ROWS=3 COLS=60>".clHtmlSpecialChars($a_content["CMS_LINKDESCR"][$typenr])."</TEXTAREA>");
