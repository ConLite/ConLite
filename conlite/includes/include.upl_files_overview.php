 ...
 *
 *   $Id: include.upl_files_overview.php 374 2015-11-09 15:59:28Z oldperl $:
 */
...
if ($action == "upl_upload" && $bDirectoryIsWritable == true) {
    ...
    if ($bUplErr) {
        $sTmpErrorMessage = $notification->returnNotification("warning", i18n("Errors while uploading file(s). Some or all files were not uploaded."));
    }
    $sErrorMessage = $sTmpErrorMessage . "<br />" . $sErrorMessage;
}
...
class UploadList extends FrontendList {
    ...
    public function convert($field, $data) {
        ...
        switch ($field) {
            ...
            case 3:
                ...
                switch (getFileExtension($data)) {
                    ...
                    case "wbmp":
                        ...
                        if (is_dbfs($data)) {
                            $data = '<a href="JavaScript:iZoom(\'' . $sess->url($cfgClient[$client]["path"]["htmlpath"] . "dbfs.php?file=" . $data) . '\');">
                            ...
                            $retValue = '<a href="JavaScript:iZoom(\'' . $cfgClient[$client]["path"]["htmlpath"] . $cfgClient[$client]["upload"] . $data . '\');">
                                 <img class="hover" name="smallImage"  onMouseOver="correctPosition(this, ' . $iWidth . ', ' . $iHeight . ');" onmouseout="if (typeof(previewHideIe6) == \'function\') {previewHideIe6(this)}" src="' . $sCacheThumbnail . '">
                                     <img class="preview" name="prevImage" src="' . $sCacheThumbnail . '">
                                         </a>';
            ...
            case 5:
                $data = human_readable_size($data);
                break;

            case 6:
                $data = uplGetFileTypeDescription($data);
                break;
        }
        return $data;
    }
 ...
 * @global type $cfg
 * @global type $client
 * @global type $cfgClient
 * @global type $area
 * @global type $frame
 * @global type $sess
 * @global type $browserparameters
 * @global type $appendparameters
 * @global type $perm
 * @global type $auth
 * @global type $sReloadScript
 * @global type $notification
 * @global type $bDirectoryIsWritable
 * @param type $path
 * @param int $sortby
 * @param string $sortmode
 * @param type $startpage
 * @param type $thumbnailmode
...
function uplRender($path, $sortby, $sortmode, $startpage = 1, $thumbnailmode, $sErrorMessage) {
    ...
    while ($item = $uploads->next()) {
        ...
        if ($bAddFile == true) {
            ...
            $list2->setData($rownum, 'bgcolor', $mark, $dirname . $filename, $showfilename, $filesize, getFileExtension($filename), $todo->render() . $actions);
            $rownum++;
        }
    }
    ...
    if ($rownum == 0) {
        ...
        $page->setContent($sErrorMessage . i18n("No files found") . $markSubItem);
        $page->render();
        return;
    }
...
uplRender($path, $sortby, $sortmode, $startpage, $thumbnailmode, $sErrorMessage);
?>